...
const MainCarousel = () => {
  const queryResults = useQuery("repoData", () =>
    fetch("http://localhost:1337/api/nuts-arabia-banners?populate=image").then((res) =>
      res.json()
    )
  )
  ...
  return (
    <>
      <Swiper
        ...
        className="mySwiper"
      >
        {/* queryResults?.data?.data? */sliderData.map((item: any, i: any) => (
          <SwiperSlide
          ...
          >
            <div className="text-sm text-gray-600 hover:text-emerald-dark">
              <Image
                ...
                width={950}
                height={400}
                src={/* "http://localhost:1337"+item.attributes.image.data[0].attributes.url */item.image}
